...
> [!NOTE]
> **动手试试**
> 1. 给 `OrderFacade` 的下单流程中间插一步"风控校验"（调用一个 `RiskService`）。验证：所有调用方代码一行都不用改，只动了外观内部。
> 2. 找出你项目里一段"调用方连续调了五六个 service、还得自己处理它们之间顺序"的代码，试着把它收进一个外观方法里。
> 3. **思考**：外观（Facade）和上一章的代理（Proxy）都"包了一层"。一句话说清它俩的根本区别是什么？再想想：外观会不会一不小心写成堆满业务逻辑的"上帝类"？怎么避免？

**王哥**（喝完最后一口冰美式）：“结构型模式的'入门四件套'——适配器、装饰器、代理、外观，你都拿下了。剩下还有三个进阶的硬骨头：桥接、组合、享元，咱们一章一个慢慢啃。”
