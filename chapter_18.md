...
> [!NOTE]
> **动手试试**
> 1. 给订单新增一个 `ShippedState`（待收货）状态：实现它的 `pay()`（提示"已发货无需支付"）和 `cancel()`（提示"已发货，需走退货流程"）。验证：你没有改动任何已有的状态类。
> 2. 给 `OrderState` 接口加一个 `confirm()`（确认收货）操作，让 `ShippedState.confirm()` 把状态切到 `CompletedState`。
> 3. **思考**：状态模式把状态流转的"规则"分散到了各个状态类里（每个状态自己知道下一站是谁）。这样做的好处是新增状态不动别人；坏处是"整张状态图"散落各处、不容易一眼看全。你会怎么补救这个可读性问题？

**王哥**：“行为型模式就剩最后几招了。给你看个场景——'**一个请求，要依次经过好几个处理者,每个处理者决定自己处理还是往后传**'——”
...
---
*下一章，责任链模式将教小白如何让请求在处理者链条上"击鼓传花"。*
