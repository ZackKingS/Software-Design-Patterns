...
> [!NOTE]
> **动手试试**
> 1. 给 `Burger.Builder` 在 `build()` 里加一条校验：肉饼数量不能为 0，否则抛异常。体会"集中校验"的好处。
> 2. 把 `Burger` 的所有字段改成 `final`、只留 getter，让它成为**不可变对象**——验证除了 Builder，没有任何地方能改它的字段。
> 3. **思考**：建造者和上一章的工厂能配合吗？想象一个 `OrderBuilder`，其中"用哪种优惠策略"这一步由一个工厂决定——画出它们的协作关系。
