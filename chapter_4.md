...
> [!NOTE]
> **动手试试**
> 1. 用**简单工厂**写一个 `PanFactory`，支持铁锅 / 不粘锅；再新增一种"麦饭石锅"，亲手感受一下你被迫改动了工厂里的 switch——这就是简单工厂违反开闭的地方。
> 2. 把上面改造成**工厂方法**（一种锅一个工厂），再加"麦饭石锅"，验证这次你只是新增了一个类、没动任何老代码。
> 3. **思考**：什么时候该用简单工厂、什么时候上工厂方法、什么时候才需要抽象工厂？用一句话各写一个判断标准。
