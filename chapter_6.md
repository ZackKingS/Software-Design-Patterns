...
> [!NOTE]
> **动手试试**
> 1. 给 `GameRole` 一个 `List<String> skills` 字段，先用**浅拷贝** `clone()` 复制两个角色，给其中一个加技能，观察另一个也跟着变——亲手复现"浅拷贝陷阱"。
> 2. 改成**深拷贝**（在 `clone()` 里 `new ArrayList<>(skills)`），验证两个角色的技能列表彻底独立。
> 3. **思考**：原型模式（clone）和建造者模式（一步步 new）都能造对象。什么时候 clone 更划算、什么时候反而是坑？（提示：对象创建成本、字段里有没有难拷贝的资源句柄。）
