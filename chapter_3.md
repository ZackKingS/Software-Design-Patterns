...
> [!NOTE]
> **动手试试**
> 1. 把 DCL 单例里的 `volatile` 去掉，查一查"指令重排序"，说清楚：为什么极端并发下，别的线程可能拿到一个"还没构造完"的半成品对象？
> 2. 用反射攻击你的 DCL 单例（`setAccessible(true)` 后 `newInstance()`），证明它能被破出第二个实例；再换成**枚举单例**，验证同样的攻击会失败。
> 3. **思考**：王哥说单例常被叫"反模式"，因为它不利于单元测试。如果一个类内部 `XxxSingleton.getInstance()` 写死了依赖，你要给它写 mock 测试会卡在哪？换成"构造器注入"会不会更好测？
