...
> [!NOTE]
> **动手试试**
> 1. 给 `DataQueryProxy` 再加一个"**统计耗时**"的能力（调用前后记时间戳、打印毫秒数）。验证 `RealDataQuery` 一行没动。
> 2. 把那个"日志"动态代理 `LogProxyHandler` 套到另一个**完全不同的接口**（比如 `UserService`）上，亲手验证"一个 Handler 代理所有接口"——这正是它比静态代理强的地方。
> 3. **思考**：JDK 动态代理只能代理"实现了接口"的类。如果一个类没有接口，Spring 是怎么给它做代理的？（提示：查一查 CGLIB 和"基于子类的代理"。）
